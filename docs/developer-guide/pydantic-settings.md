...
- **`env_file`** — A tuple of dotenv files loaded in order. Later files have
  *higher* priority: `/run/configs/app-config` (Docker Swarm Config / K8s
  ConfigMap) is lowest, then `.env.ddx` (the ddx-generated dev service ports),
  then `.env` which wins. pydantic-settings reads all of them and merges them
  into a single dict, with later files overriding earlier ones.
...
3. .env.ddx                            ← ddx-generated dev service ports
4. /run/configs/app-config             ← Docker Swarm Config / K8s ConfigMap
5. /run/secrets/<fieldname>            ← Docker Swarm Secret / K8s Secret volume
6. .secrets/<fieldname>                ← Local secrets directory
7. _EnvDefaultsSource                  ← Dev / devcontainer / prod defaults
8. Field-level Python default          ← e.g., port: int = 5432 (lowest)
```
...
| 3 | `.env.ddx` | Generated by `ddx dev` (service ports) | Automatic | Not present |
| 4 | `/run/configs/app-config` | Config file mounted by orchestrator | Not present | Used in Docker/K8s |
| 5 | `/run/secrets/<name>` | Secret file mounted by orchestrator | Not present | Used in Docker/K8s |
| 6 | `.secrets/<name>` | Per-file secrets (`0o600`) | Main dev source | Not typically used |
| 7 | `_EnvDefaultsSource` | Classmethod defaults | `get_dev_defaults()` | `get_prod_defaults()` |
| 8 | Field default | `field: type = value` | Fallback | Fallback |

The key insight: **levels 1-6 are identical in dev and prod**. The difference
is that dev provides safe defaults at level 7, while prod requires values
from levels 1-6.
...
| 3 | `.env.ddx` | `.env.prod` |
| 4 | `get_dev_defaults()` | `get_prod_defaults()` |
| 5 | **MISSING** | **MISSING** |
