...
def test_django_simple_history_install_and_remove(temp_dir):
    ...
    result = runner.invoke(app, ["packages", "add", "django_simple_history"])
    assert result.exit_code == 0, f"Install failed: {result.output}"

    settings_file = temp_dir / "settings" / "packages" / "django_simple_history.py"
    ...
    assert PixiRunner().has_dependency("django-simple-history")

    result = runner.invoke(app, ["packages", "remove", "django_simple_history"])
    assert result.exit_code == 0, f"Remove failed: {result.output}"

    assert not settings_file.exists(), "Settings file not removed"
    assert not PixiRunner().has_dependency("django-simple-history")
