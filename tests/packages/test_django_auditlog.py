...
def test_django_auditlog_install_and_remove(temp_dir):
    ...
    settings_file = temp_dir / "settings" / "packages" / "django_auditlog.py"
    ...
    assert PixiRunner().has_dependency("django-auditlog"), (
    ...
    assert not PixiRunner().has_dependency("django-auditlog"), (
