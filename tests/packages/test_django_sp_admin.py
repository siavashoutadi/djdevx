...
def test_django_sp_admin_install_and_remove(temp_dir):
    ...
    result = runner.invoke(app, ["packages", "add", "django_sp_admin"])
    assert result.exit_code == 0, f"Install failed: {result.output}"

    settings_file = temp_dir / "settings" / "packages" / "django_sp_admin.py"
    assert settings_file.exists(), "Settings file not created"

    expected_content = (
        DATA_DIR / "settings" / "packages" / "django_sp_admin.py"
    ).read_text()
    assert settings_file.read_text() == expected_content, "Settings content mismatch"

    result = runner.invoke(app, ["packages", "remove", "django_sp_admin"])
