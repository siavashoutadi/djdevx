...
def test_whitenoise_plus_s3(temp_dir):
    ...
    whitenoise_file = temp_dir / "settings" / "packages" / "whitenoise.py"
    ...
    result = runner.invoke(
        app, ["packages", "add", "django-storages", "--provider", "s3"]
    )
    assert result.exit_code == 0, f"S3 install failed: {result.output}"

    s3_file = temp_dir / "settings" / "packages" / "django_storages_s3.py"
    ...
    assert PixiRunner().has_dependency("whitenoise")
...
def test_whitenoise_plus_azure(temp_dir):
    ...
    result = runner.invoke(
        app, ["packages", "add", "django-storages", "--provider", "azure"]
    )
    assert result.exit_code == 0, f"Azure install failed: {result.output}"

    azure_file = temp_dir / "settings" / "packages" / "django_storages_azure.py"
...
def test_whitenoise_plus_google(temp_dir):
    ...
    result = runner.invoke(
        app, ["packages", "add", "django-storages", "--provider", "google"]
    )
    assert result.exit_code == 0, f"Google install failed: {result.output}"

    google_file = temp_dir / "settings" / "packages" / "django_storages_google.py"
