...
def test_django_permissions_policy_install_and_remove(temp_dir):
    ...
    result = runner.invoke(app, ["packages", "add", "django_permissions_policy"])
    assert result.exit_code == 0, f"Install failed: {result.output}"

    settings_file = temp_dir / "settings" / "packages" / "django_permissions_policy.py"
    ...
    assert PixiRunner().has_dependency("django-permissions-policy")

    result = runner.invoke(app, ["packages", "remove", "django_permissions_policy"])
    assert result.exit_code == 0, f"Remove failed: {result.output}"

    assert not settings_file.exists(), "Settings file not removed"
    assert not PixiRunner().has_dependency("django-permissions-policy")
