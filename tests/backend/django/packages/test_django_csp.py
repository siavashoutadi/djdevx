...
def test_django_csp_install_and_remove(temp_dir):
    ...
    settings_file = backend_dir / "settings" / "packages" / "django_csp.py"
    ...
    assert DjangoProjectManager().has_dependency("django-csp"), (
    ...
    assert not DjangoProjectManager().has_dependency("django-csp"), (
