...
def test_django_debug_toolbar_install_and_remove(temp_dir):
    ...
    settings_file = backend_dir / "settings" / "packages" / "django_debug_toolbar.py"
    assert settings_file.exists(), "Settings file not created"

    urls_file = backend_dir / "urls" / "packages" / "django_debug_toolbar.py"
    ...
    assert DjangoProjectManager().has_dependency("django-debug-toolbar", "dev"), (
    ...
    assert not DjangoProjectManager().has_dependency("django-debug-toolbar", "dev"), (
