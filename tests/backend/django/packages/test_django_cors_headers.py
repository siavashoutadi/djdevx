...
def test_django_cors_headers_install_and_remove(temp_dir):
    ...
    settings_file = backend_dir / "settings" / "packages" / "django_cors_headers.py"
    ...
    assert DjangoProjectManager().has_dependency("django-cors-headers"), (
    ...
    assert not DjangoProjectManager().has_dependency("django-cors-headers"), (
