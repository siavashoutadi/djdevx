...
def test_django_browser_reload_install_and_remove(temp_dir):
    ...
    settings_file = backend_dir / "settings" / "packages" / "django_browser_reload.py"
    assert settings_file.exists(), "Settings file not created"

    urls_file = backend_dir / "urls" / "packages" / "django_browser_reload.py"
    ...
    assert DjangoProjectManager().has_dependency("django-browser-reload", "dev"), (
    ...
    assert not DjangoProjectManager().has_dependency("django-browser-reload", "dev"), (
