...
def test_django_filter_install_and_remove(temp_dir):
    ...
    settings_file = backend_dir / "settings" / "packages" / "django_filter.py"
    ...
    assert DjangoProjectManager().has_dependency("django-filter"), (
    ...
    assert not DjangoProjectManager().has_dependency("django-filter"), (
