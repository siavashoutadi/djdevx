...
_ENV_DDX_HEADER = "# Generated by ddx — dev service ports. Do not edit by hand."
...
class BaseDevService(ABC):
    ...
    def _set_port_env(self, quiet: bool = False, step=None) -> None:
        ...
        Also publishes the port into the generated ``.env.ddx`` file so plain
        ``pixi run python manage.py ...`` invocations (which never see this
        process's environment) resolve the same port through pydantic-settings.
    ...
    def _publish_port_to_env_ddx(self) -> None:
        """Upsert this service's ``PORT_KEY=port`` line in ``.env.ddx``."""
        if not self.port_env_key:
            return
        self._write_env_ddx({self.port_env_key: str(self.port)})

    def _write_env_ddx(self, updates: dict[str, str]) -> None:
        """Merge *updates* into ``.env.ddx``, preserving other ``KEY=VALUE`` lines."""
        path = self.structure.env_ddx_path
        ...
        lines = [_ENV_DDX_HEADER] + [f"{key}={value}" for key, value in entries.items()]
        path.write_text("\n".join(lines) + "\n")

    def _remove_from_env_ddx(self) -> None:
        """Drop this service's port line from ``.env.ddx`` (used by purge)."""
        if not self.port_env_key:
            return
        path = self.structure.env_ddx_path
        if not path.exists():
        ...
        lines = [
            line
            for line in path.read_text().splitlines()
            if not line.strip().startswith(f"{self.port_env_key}=")
        ]
